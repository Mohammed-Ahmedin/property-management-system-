...
export default function RoomGallery({ images, roomName, services }: RoomGalleryProps) {
  ...
  return (
    <>
      ...
      {galleryOpen && (
        <div className="fixed inset-0 z-[9998] flex items-start justify-center sm:pt-8 sm:px-4"
          onClick={() => { setGalleryOpen(false); setGalleryIdx(0); }}>
          <div className="absolute inset-0 bg-black/70" />
          <div className="relative bg-white dark:bg-zinc-900 rounded-none sm:rounded-2xl shadow-2xl flex w-full max-w-5xl h-full sm:max-h-[90vh] overflow-hidden"
            onClick={(e) => e.stopPropagation()}>
            <div className="flex-1 flex flex-col overflow-hidden min-w-0">
              {/* Header */}
              <div className="flex items-center gap-2 px-3 sm:px-4 py-2.5 sm:py-3 border-b border-gray-200 dark:border-zinc-700 shrink-0">
                {galleryIdx > 0 && (
                ...
                <button className="flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full border-2 border-primary text-primary text-xs sm:text-sm font-medium">
                  <ImageIcon className="w-3.5 h-3.5" /> <span className="hidden sm:inline">Room Images</span><span className="sm:hidden">Photos</span>
                </button>
              ...
              {galleryIdx > 0 ? (
                <div className="flex-1 flex flex-col overflow-hidden">
                  <div className="flex-1 relative bg-black flex items-center justify-center min-h-0">
                    <button className="absolute left-2 sm:left-3 top-1/2 -translate-y-1/2 text-white p-2 sm:p-2.5 bg-black/40 hover:bg-black/60 rounded-full z-10"
                      onClick={() => setGalleryIdx(i => Math.max(1, i - 1))}>
                      <ChevronLeft className="w-4 h-4 sm:w-5 sm:h-5" />
                    </button>
                    <img src={images[galleryIdx - 1]?.url} alt="" className="max-h-full max-w-full object-contain" />
                    <button className="absolute right-2 sm:right-3 top-1/2 -translate-y-1/2 text-white p-2 sm:p-2.5 bg-black/40 hover:bg-black/60 rounded-full z-10"
                      onClick={() => setGalleryIdx(i => Math.min(images.length, i + 1))}>
                      <ChevronRight className="w-4 h-4 sm:w-5 sm:h-5" />
                    </button>
                  </div>
                  <div className="h-[56px] sm:h-[72px] bg-black flex items-center gap-1 px-2 overflow-x-auto shrink-0">
                    <button onClick={() => setGalleryIdx(0)} className="flex flex-col items-center gap-0.5 px-2 py-1 text-white text-xs shrink-0">
                      <div className="w-6 h-6 sm:w-7 sm:h-7 bg-white/20 rounded flex items-center justify-center">
                    ...
                    {images.map((img, i) => (
                      <button key={img.id} onClick={() => setGalleryIdx(i + 1)}
                        className={`shrink-0 h-[40px] w-[56px] sm:h-[56px] sm:w-[72px] rounded overflow-hidden border-2 transition-colors ${galleryIdx === i + 1 ? "border-primary" : "border-transparent opacity-70 hover:opacity-100"}`}>
