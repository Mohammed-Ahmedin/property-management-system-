...
import { ArrowLeft, Calendar, CreditCard, Download, Loader2, Package, BedDouble, CheckCircle2, Clock, Star, Share2, Printer } from "lucide-react";
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-4">
        ...
        <div className="relative rounded-3xl overflow-hidden mb-6 shadow-2xl">
          ...
          <div className="absolute bottom-0 left-0 right-0 p-5 md:p-8">
            ...
            <h1 className="text-xl md:text-3xl font-bold text-white mb-2">{booking.property?.name}</h1>
          ...
          {[
            ...
            <div key={step.label} className="flex items-center flex-1 min-w-0">
              <div className="flex flex-col items-center gap-1 flex-1 min-w-[60px]">
                <div className={cn("w-8 h-8 rounded-full flex items-center justify-center", step.done ? "bg-primary text-white" : "bg-muted text-muted-foreground")}>
        ...
        {/* Main layout: flex-row on lg+, stacked on mobile */}
          ...
          <div className="flex-1 min-w-0 space-y-5">
            ...
            <div className="bg-card border border-border rounded-2xl p-5 md:p-7">
              <h3 className="font-semibold text-base flex items-center gap-2 mb-5">
                <Calendar className="w-5 h-5 text-primary" /> Your Stay
              </h3>
            ...
            <div className="bg-card border border-border rounded-2xl p-5 md:p-7">
              ...
              <div className="flex items-center gap-4">
                {roomImg && <img src={roomImg} alt={booking.room?.name} className="w-20 h-20 rounded-xl object-cover shrink-0" />}
                <div>
                  <p className="font-bold text-base">{booking.room?.name}</p>
                  {(booking.room as any)?.description && (
                    <p className="text-sm text-muted-foreground mt-1 line-clamp-2">{(booking.room as any).description}</p>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-2xl p-5 md:p-7">
              ...
              {[
                ...
                <div key={label} className="flex justify-between items-center text-sm py-3 border-b border-border/50 last:border-0">
            ...
            {(booking.additionalServices?.length ?? 0) > 0 && (
              <div className="bg-card border border-border rounded-2xl p-5 md:p-7">
                <h3 className="font-semibold text-base flex items-center gap-2 mb-5">
                  <Package className="w-5 h-5 text-primary" /> Additional Services
                </h3>
                <ul className="space-y-2">
                  {booking.additionalServices.map((s: any, i: number) => (
                    <li key={i} className="flex justify-between items-center text-sm py-3 px-4 rounded-xl bg-muted/40 border border-border/50">
            ...
            <div className="lg:sticky lg:top-20 space-y-4">
              ...
              <div className="bg-card border border-border rounded-2xl overflow-hidden shadow-lg">
                <div className="bg-gradient-to-br from-primary to-primary/80 p-6 text-primary-foreground">
                  ...
                  <FormatedAmount amount={Number(booking.totalAmount)} className="text-4xl font-bold text-white" />
